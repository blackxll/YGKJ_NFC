...
void RW_NDEF_T2T_Write_Next(unsigned char *pRsp, unsigned short Rsp_size, unsigned char *pCmd, unsigned short *pCmd_size)
{
	...
	*pCmd_size = 0;
	...
	case Reading_CC:
		...
		{
			/* Is size enough ? */
			if (pRsp[2]*8 >= NdefMessage_size)
			{
				/* Write First data */
				pCmd[0] = 0xA2;
				pCmd[1] = 0x04;
				pCmd[2] = 0x03;
				if (NdefMessage_size > 0xFF)
				{
					pCmd[3] = 0xFF;
					pCmd[4] = (NdefMessage_size & 0xFF00) >> 8;
					pCmd[5] = NdefMessage_size & 0xFF;
					RW_NDEF_T2T_Ndef.MessagePtr = 0;
				}
				else
				{
					pCmd[3] = NdefMessage_size;
					memcpy(&pCmd[4], pNdefMessage, 2);
	...
	case Writing_Data:
		...
		if ((Rsp_size == 2) && (pRsp[Rsp_size-1] == 0x00))
		{
			...
			if (NdefMessage_size <= RW_NDEF_T2T_Ndef.MessagePtr)
			{
				/* Notify application of the NDEF reception */
				if(pRW_NDEF_PushCb != NULL) pRW_NDEF_PushCb(pNdefMessage, NdefMessage_size);
			}
			else
			{
				/* Write NDEF content */
				pCmd[0] = 0xA2;
				pCmd[1] = RW_NDEF_T2T_Ndef.BlkNb;
				memcpy(&pCmd[2], pNdefMessage+RW_NDEF_T2T_Ndef.MessagePtr, 4);
				*pCmd_size = 6;
